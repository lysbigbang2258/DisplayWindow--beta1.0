...
namespace ArrayDisplay.NetUtl
{
    ...
    public class UdpWaveData : IDisposable
    {
        ...
        private readonly List<byte[]> recTmpBytes = new List<byte[]>();
        private readonly List<byte[]> recSendBytes = new List<byte[]>();
        private readonly List<byte> saveBytes = new List<byte>();
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (disposing)
            {
                if (waveSocket != null)
                {
                    try
                    {
                        waveSocket.Shutdown(how: SocketShutdown.Both);
                        Thread.Sleep(10);
                    }
                    catch (Exception e)
                    {
                        LogHelper.LogInfo(info: e.Message);
                        throw;
                    }
            ...
            while (true)
            {
                ...
                for (int index = 0; index < FrameNums; index++)
                {
                    ...
                    try
                    {
                        ...
                        if (ret != 0)
                        {
                            var rcvBytes = new byte[ConstUdpArg.WORK_FRAME_LENGTH];
                ...
                if (recTmpBytes.Count > FrameNums * 10)
                {
                    try
                    {
                        lock (lockobj)
                        ...
                        if (PutWorkData(recSendBytes.ToArray(),ref Splikbytes))
                        {
                            WorkDataEventHandler?.Invoke(null, e: Splikbytes);
                            List<byte> temp = new List<byte>();
                            foreach (byte[] recSendByte in recSendBytes)
                            {
                                temp.AddRange(recSendByte);
                            }
                            WorkSaveDataEventHandler?.Invoke(null,temp.ToArray());
                        }
                    }
                    catch (Exception e)
                    {
        ...
        private bool PutWorkData(byte[][] buf,ref byte[][] outBytes)
        {
            if (buf == null)
            {
                return false;
            ...
            int inLen = buf.Length * buf[0].Length;

            int packLen = inLen / ConstUdpArg.ARRAY_NUM;


            outBytes = new byte[ConstUdpArg.ARRAY_NUM][];
            for (int i = 0; i < outBytes.Length; i++)
            {
                outBytes[i] = new byte[packLen * 4];
            }
            ...
            try
            {
                for (int index = 0; index < buf.Length; index++)
                {
                    ...
                    {
                        Array.Copy(buf[index], i * 4, outBytes[i], index * 4, 4);
                    }
                }
            }
            catch (Exception e)
